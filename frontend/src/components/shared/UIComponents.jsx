...
export const StatCard = ({ icon, title, label, value, color }) => (
    <Card>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
                <p style={{ color: '#5f6368', fontSize: '0.875rem', margin: '0 0 8px 0' }}>{title || label}</p>
                <h2 style={{ fontSize: '2rem', fontWeight: 700, margin: 0, color: '#202124' }}>{value}</h2>
            </div>
            {icon && <div style={{ color: color || 'var(--primary)' }}>{icon}</div>}
        </div>
    </Card>
);
